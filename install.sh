...
#
# Containers: NPMplus, Technitium DNS, Portracker, WireGuard, Wireshark, Homarr
#
...
volumes:
  ...
  portracker_data:
    ...
    driver_opts:
      type: none
      o: bind
      device: ${DATA_DIR}/portracker/data
  wireguard_config:
    driver: local
    driver_opts:
      type: none
...
services:
  ...
  # Portracker -- port tracking dashboard
  # App listens on container port 4999 (not 3000)
  portracker:
    ...
    networks: [netstack]
    ports:
      - "\${PORTRACKER_PORT}:4999"
    volumes:
      - portracker_data:/app/data
    environment:
      - TZ=\${TZ}
...
ufw allow 8072/tcp  comment "Portracker"    > /dev/null 2>&1
